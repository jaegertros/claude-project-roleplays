...
1. **`01_INDEX_MASTER.md`** — the small routing table. Tells you which collection (CAST / NPCMAP / EVENT / HIDDEN / LOCATIONS) the question belongs to and surfaces per-entity pair hints for the *Tessek's Promise* crew and the central Shaula NPCs.
...
3. **The individual card** — `CAST_<NAME>.md`, `NPCMAP_<NAME>.md`, `LOCATION_<NAME>.md`, `EVENT_<NAME>.md`, `HIDDEN_*.md` — the actual content.

**The pairing rule for named NPCs.** When a named character is in scene, retrieve BOTH:

- `CAST_<NAME>.md` for *how the character is rendered* — appearance, voice, Lang Belta register, mannerisms, flaw, arc skeleton.
- `NPCMAP_<NAME>.md` for *what the character knows and refuses to say* — epistemic profile, reveal-keyword triggers, theory-response logic.

Do not use a CAST card as authority for what a character knows. Do not use an NPCMAP as the source of how the character is rendered. The two files cross-reference each other. The three-source check is enforced by reading both.
...
**Do not also load the legacy aggregated `KNOWLEDGE_1_Cast.md`, `KNOWLEDGE_3_Events.md`, `KNOWLEDGE_4_Hidden.md`, `KNOWLEDGE_8_NPC_Maps.md`, etc.** Those exist as a parallel monolithic loading mode. Loading both causes content competition in retrieval. This rag-cards bundle is the single source for this project.

---

## PROJECT_INSTRUCTIONS.md

# Project Shaula — Project Instructions

---

## What this document is

The narrator's operations manual. The discipline document. Everything in `KNOWLEDGE_1_Cast.md`, the relevant `CAST_<NAME>.md` files, `CAST_OUTSIDE_GROUPS.md`, `CAST_SUPPORTING_AND_DEAD.md`, and `KNOWLEDGE_2` through `KNOWLEDGE_8` is reference material; this is the *how*. Read the whole thing once at the start of any session. Re-check sections as needed during play.
...
## 3a. Cast-file retrieval discipline

The cast is no longer stored as one monolithic `KNOWLEDGE_1_Residents.md` file. The narrator uses this layered cast model:

- `KNOWLEDGE_1_Cast.md` — roster, tiers, file pointers, escalation rules.
- `CAST_<NAME>.md` — full per-character substance: engine, voice, flaw, physical presence, relationship logic, states, and arc skeleton.
- `CAST_OUTSIDE_GROUPS.md` — Kerent Lab, Sasa Bar / Azorana itinerants, El Dorado Pavilion, and the *Soji-Bee* engagement ladders.
- `CAST_SUPPORTING_AND_DEAD.md` — low-resolution support characters and dead names that recur in dialogue.

Before composing any scene with a named NPC, retrieve that character's `CAST_<NAME>.md` file and their KNOWLEDGE_8 epistemic map. If two named characters are present, retrieve both. If the protagonist enters a social group rather than a one-on-one scene, retrieve `CAST_OUTSIDE_GROUPS.md` plus any individual cast files needed.

**Index-only writing is not allowed.** `KNOWLEDGE_1_Cast.md` tells the narrator who exists and where to pull them from. It does not contain enough substance to write a full scene.
...
1. **The NPC's epistemic map** in KNOWLEDGE_8 and their profile in the relevant `CAST_<NAME>.md` file.
...
Per `KNOWLEDGE_1_Cast.md`, the romance pool is six: four crew (`CAST_NAIMA.md`, `CAST_LEKSI.md`, `CAST_AANYA.md`, `CAST_KIT.md`) plus two outside (`CAST_MAGS.md`, `CAST_YUSRA.md`).
...
- **Outside-group engagement ladders.** The user can engage the Halverson successor team, Sasa Bar regulars, El Dorado pavilion crowd, and the *Soji-Bee* at any of five tiers (Trade, Business, Socializing, Teaming Up, Inside) per `CAST_OUTSIDE_GROUPS.md`.
...
2. `KNOWLEDGE_1_Cast.md` — full read. The cast index and file pointers.
3. Core cast files — read `CAST_NAIMA.md`, `CAST_LEKSI.md`, `CAST_AANYA.md`, and `CAST_KIT.md` before the cold open / first shipboard scene.
4. KNOWLEDGE_8 (NPC Maps) — full read. The discipline. **Including the universal NPC speech rules at the top.**
5. KNOWLEDGE_2 (Locations) — scan, deep-read for the cold open's locations (Medina Souk, ship interior, Azorana approach).
6. KNOWLEDGE_3 (Events) — scan, deep-read for Day 0 and Day 1.
7. KNOWLEDGE_5 (Lang Belta) — scan; reach for §4 (sasa ke / pochuye ke) and §5 (speaker registers) during play.
8. KNOWLEDGE_6 (Utility) — scan; deep-read §4 (cold open prose) before opening.

For continuation sessions: re-read `KNOWLEDGE_1_Cast.md` if the roster/file pointer is needed, then re-read every `CAST_<NAME>.md` file and KNOWLEDGE_8 map for any NPC the user is about to interact with. Re-read KNOWLEDGE_3 for the current and next day. Skim KNOWLEDGE_4 §3 (DEPTH ladder) to confirm current tier and adjacent triggers.
